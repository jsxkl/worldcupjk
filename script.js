(function () {
    ...
    const openDialog = (matchId, home, away) => {
        ...
        if (pred.handicap && !pred.handicap.picks && pred.handicap.pick) {
            pred.handicap.picks = [pred.handicap.pick];
        }
        const form = `
      <div style="background:#1a2f3f; padding:1.5rem; border-radius:1.5rem; max-width:400px; margin:2rem auto; color:#fff; box-shadow: 0 25px 50px rgba(0,0,0,0.5);">
        <h3 style="margin-bottom:1rem;">📝 ${home} vs ${away}</h3>
        <div style="margin:0.6rem 0;"><label>比分 (主-客): </label><input id="ps" type="text" placeholder="2-1" value="${pred.score ? pred.score.home + '-' + pred.score.away : ''}" style="width:90px; background:#0f1e2f; border:1px solid #ffd966; color:#fff; padding:0.3rem; border-radius:0.5rem;"></div>
        <div style="margin:0.6rem 0;"><label>总进球: </label><input id="pt" type="number" min="0" value="${pred.totalGoals ?? ''}" style="width:70px; background:#0f1e2f; border:1px solid #ffd966; color:#fff; padding:0.3rem; border-radius:0.5rem;"></div>
        <div style="margin:0.6rem 0;"><label>胜负: </label>
          <select id="pw" style="background:#0f1e2f; border:1px solid #ffd966; color:#fff; padding:0.3rem; border-radius:0.5rem;">
            <option value="">不猜</option><option value="home" ${pred.winner === 'home' ? 'selected' : ''}>主胜</option><option value="away" ${pred.winner === 'away' ? 'selected' : ''}>客胜</option><option value="draw" ${pred.winner === 'draw' ? 'selected' : ''}>平局</option>
          </select>
        </div>
        <div style="margin:0.6rem 0;">
          <label>让球数: </label>
          <input id="phl" type="number" step="0.5" value="${pred.handicap ? pred.handicap.line : ''}" style="width:70px; background:#0f1e2f; border:1px solid #ffd966; color:#fff; padding:0.3rem; border-radius:0.5rem;" placeholder="例如1">
          <span style="color:#94a3b8; font-size:0.7rem;">（主队让球，负数=受让）</span>
        </div>
        <div style="margin:0.6rem 0;">
          <label>让球结果: </label>
          <div class="handicap-options">
            <label class="handicap-option ${(pred.handicap?.picks || []).includes('home') ? 'active' : ''}">
              <input type="checkbox" id="chk_home" ${(pred.handicap?.picks || []).includes('home') ? 'checked' : ''}> 让球主胜
            </label>
            <label class="handicap-option ${(pred.handicap?.picks || []).includes('draw') ? 'active' : ''}">
              <input type="checkbox" id="chk_draw" ${(pred.handicap?.picks || []).includes('draw') ? 'checked' : ''}> 让球平
            </label>
            <label class="handicap-option ${(pred.handicap?.picks || []).includes('away') ? 'active' : ''}">
              <input type="checkbox" id="chk_away" ${(pred.handicap?.picks || []).includes('away') ? 'checked' : ''}> 让球客胜
            </label>
          </div>
        </div>
    ...
    `;
        ...
        document.getElementById('saveBtn').onclick = () => {
            ...
            const t = parseInt(document.getElementById('pt').value);
            if (!isNaN(t) && t >= 0) data.totalGoals = t;
            const w = document.getElementById('pw').value; if (w) data.winner = w;
            const hl = parseFloat(document.getElementById('phl').value);
            const picks = [];
            if (document.getElementById('chk_home').checked) picks.push('home');
            if (document.getElementById('chk_draw').checked) picks.push('draw');
            if (document.getElementById('chk_away').checked) picks.push('away');
            if (!isNaN(hl) && picks.length > 0) {
                data.handicap = { line: hl, picks: picks };
            }
    ...
    const renderMatches = (events) => {
        ...
        events.forEach(ev => {
            ...
            if (pred && pred.handicap && !pred.handicap.picks && pred.handicap.pick) {
                pred.handicap.picks = [pred.handicap.pick];
            }
            ...
            if (!pred) {
                ...
                if (pred.totalGoals !== undefined) {
                    const ok = isEnd && pred.totalGoals === h + a;
                    rows += `<div class="guess-row"><span class="guess-label">总进球</span><span class="${ok ? 'correct' : (isEnd ? 'wrong' : 'pending')}">${pred.totalGoals} ${isEnd ? (ok ? '✅' : '❌') : ''}</span></div>`;
                }
                if (pred.winner) {
                    const actual = h > a ? 'home' : (h < a ? 'away' : 'draw');
                    const ok = isEnd && pred.winner === actual;
                    const map = { home: '主胜', away: '客胜', draw: '平局' };
                    rows += `<div class="guess-row"><span class="guess-label">胜负</span><span class="${ok ? 'correct' : (isEnd ? 'wrong' : 'pending')}">${map[pred.winner]} ${isEnd ? (ok ? '✅' : '❌') : ''}</span></div>`;
                }
                ...
                if (pred.score) total++;
                if (pred.totalGoals !== undefined) total++;
                if (pred.winner) total++;
                if (pred.handicap && pred.handicap.picks && pred.handicap.picks.length > 0) total++;
                if (isEnd) {
                    if (pred.score) { finished++; if (pred.score.home === h && pred.score.away === a) correct++; }
                    if (pred.totalGoals !== undefined) { finished++; if (pred.totalGoals === h + a) correct++; }
                    if (pred.winner) { finished++; if (pred.winner === (h > a ? 'home' : (h < a ? 'away' : 'draw'))) correct++; }
                    if (pred.handicap && pred.handicap.picks && pred.handicap.picks.length > 0) { finished++; const adj = h - pred.handicap.line; const actual = adj > a ? 'home' : (adj < a ? 'away' : 'draw'); if (pred.handicap.picks.includes(actual)) correct++; }
                }
            }
